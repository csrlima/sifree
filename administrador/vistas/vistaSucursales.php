...
<script type="text/javascript">
    //funcion para ir a detalle
     function irDetalle(id) {
        ...
        $('#resultado').load('administrador/dialogs/dialogDetalleSucursales.php', {id: id}, function() {
    ...
    function nuevoProg() {
        ...
        $('.chk').each(function() {
            if ($(this).is(':checked')) {
                ...
                //llamado al form por ajax
                $('#cargando').css("display", "block");
                $('#resultado').load('administrador/forms/frmSoporteProgramado.php', {evento: 'agregar', id: id}, function() {
                    $("#cargando").css("display", "none");
                });
                return;
            }
    ...
    function filtrar() {
        ...
        $('#cargando').css("display", "block");
        $('#section').load('administrador/vistas/vistaSucursales.php', {filtro: id}, function() {
            $("#cargando").css("display", "none");
        });
    }
    ...
    function nuevo() {
        //llamado al form por ajax
        $('#cargando').css("display", "block");
        $('#resultado').load('administrador/forms/frmSucursales.php', {evento: 'agregar'}, function() {
            $("#cargando").css("display", "none");
        });
    }
    //llamado al form agregar nuevo registro
    ...
    function historialSoporte() {
        $('.chk').each(function() {
            if ($(this).is(':checked')) {
                ...
                //llamado al form por ajax
                $('#cargando').css("display", "block");
                $('#resultado').load('administrador/dialogs/dialogHistorialSoporteSucursal.php', {evento: 'agregar', id: id}, function() {
                    $("#cargando").css("display", "none");
                });
                return;
            }
    ...
    function editar() {
        $('.chk').each(function() {
            if ($(this).is(':checked')) {
                ...
                //llamado al form por ajax
                $('#cargando').css("display", "block");
                $('#resultado').load('administrador/forms/frmSucursales.php', {evento: 'modificar', id: id}, function() {
                    $("#cargando").css("display", "none");
                });
                return;
            }
    ...
    function eliminar() {
        smoke.confirm('¿Esta seguro que desea eliminar el (los) elemento(s)?', function(event) {
            if (event) {
                ...
                $('#cargando').css("display", "block");
                $('#resultado').load('administrador/scripts/scSucursales.php', {evento: 'eliminar', matrizEliminar: matrizEliminar}, function() {
                    $("#cargando").css("display", "none");
                });
...
<div style="min-width: 250px" class="container-top" id="container-top-right">  
