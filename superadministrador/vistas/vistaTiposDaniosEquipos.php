...
<script type="text/javascript">
    ...
    function nuevo() {
        ...
        $('#resultado').load('superadministrador/forms/frmTiposDaniosEquipos.php', {evento: 'agregar'}, function() {
    ...
    function editar() {
        $('.chk').each(function() {
            if ($(this).is(':checked')) {
                ...
                //llamado al form por ajax
                $('#cargando').css("display", "block");
                $('#resultado').load('superadministrador/forms/frmTiposDaniosEquipos.php', {evento: 'modificar', id: id}, function() {
                    $("#cargando").css("display", "none");
                });
                return;
            }
    ...
    function eliminar() {
        smoke.confirm('¿Esta seguro que desea eliminar el (los) elemento(s)?', function(event) {
            if (event) {
                ...
                $('#cargando').css("display", "block");
                $('#resultado').load('superadministrador/scripts/scTiposDaniosEquipos.php', {evento: 'eliminar', matrizEliminar: matrizEliminar}, function() {
                    $("#cargando").css("display", "none");
                });
...
<div class="container-top"> 
    <div class="print-section">
        <!--area imprimible-->
        <h2>Vista de Tipos de Daños en Equipos</h2> 
    </div>
    <!--botonera de opciones-->
    <button type="button" class="btn btn-default" onclick="nuevo()">
        <span class="glyphicon glyphicon-plus-sign"></span> Nuevo Tipo de Daño</button>
        ...
        <table  class="table table-condensed table-striped">
            <thead>
                <tr>
                    ...
                    <th>Tipo de Daño en Equipo</th>
                    <th style="width: 250px">Total Equipos Dañados</th>
                </tr>
            </thead>
            <tbody>
                ...
                <?php
                    $base = new mySQLData();
                $result = $base->consulta("SELECT * FROM tipos_danios ORDER BY id_tipo_danio");
                while ($row = mysql_fetch_array($result)) {
                    ...
                    <tr>
                        ...
                        <td>
						<?php
                        $result2 = $base->consulta("SELECT DISTINCT equipos.id_equipo FROM diagnostico_danios,equipos
							WHERE equipos.id_equipo=diagnostico_danios.id_equipo
							AND estado_equipo <=> 'Correcto'
							AND id_tipo_danio='$row[0]'");
