...
<script>
  //llamado al form agregar nuevo registro
    function asignar() {
        ...
        $('#resultado2').load('superadministrador/forms/frmAsignarEquipoSucursal.php', {evento: 'agregar',id:id}, function() {
    ...
    function devolver() {
        smoke.confirm('¿Esta seguro que desea DEVOLVER AL INVENTARIO el (los) elemento(s)?', function(event) {
            if (event) {
                ...
                $('#cargando').css("display", "block");
                $('#resultado2').load('superadministrador/scripts/scAsignarDevolverSucursal.php', {evento: 'devolver',id: id, matriz: matriz}, function() {
                    $("#cargando").css("display", "none");
                });
            }
        });
