...
<script type="text/javascript">
    ...
    function nuevo() {
        ...
        $('#resultado').load('superadministrador/forms/frmRecomendacionesAcciones.php', {evento: 'agregar'}, function() {
    ...
    function editar() {
        $('.chk').each(function() {
            if ($(this).is(':checked')) {
                ...
                //llamado al form por ajax
                $('#cargando').css("display", "block");
                $('#resultado').load('superadministrador/forms/frmRecomendacionesAcciones.php', {evento: 'modificar', id: id}, function() {
                    $("#cargando").css("display", "none");
                });
                return;
            }
    ...
    function eliminar() {
        smoke.confirm('¿Esta seguro que desea eliminar el (los) elemento(s)?', function(event) {
            if (event) {
                ...
                $('#cargando').css("display", "block");
                $('#resultado').load('superadministrador/scripts/scRecomendacionesAccionesSoporte.php', {evento: 'eliminar', matrizEliminar: matrizEliminar}, function() {
                    $("#cargando").css("display", "none");
                });
...
<div class="container-top"> 
    <div class="print-section">
        <!--area imprimible-->
        <h2>Vista Recomendaciones Acciones de Soporte</h2> 
    </div>
    <!--botonera de opciones-->
    <button type="button" class="btn btn-default" onclick="nuevo()">
        <span class="glyphicon glyphicon-plus-sign"></span> Nueva Recomendación</button>
        ...
        <table  class="table table-condensed table-striped">
            <thead>
                <tr>
                    ...
                    <th style="width:325px">Nombre Recomendación</th>
                    <th>Descripción</th>
                </tr>
            </thead>
            <tbody>
